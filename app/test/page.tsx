...
const page = () => {
  const { sendMessage, lastMessage, readyState } = useWebSocket('ws://185.251.88.75:8000/ws/room/93/');
  ...
  const handleAnswerCall = () => {
    sendMessage(JSON.stringify({ type: 'answer' }))
    setIsInCall(true)
    setIsCalling(false)
  }
  const handleDeclineCall = () => {
    sendMessage(JSON.stringify({ type: 'decline' }))
    setIsCalling(false)
    setIsInCall(false)
  }
  return (
